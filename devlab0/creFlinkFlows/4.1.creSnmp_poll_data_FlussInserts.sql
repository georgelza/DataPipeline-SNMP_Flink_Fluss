...
SET 'pipeline.name' = 'Push SNMP snmp_poll_data4 => fluss.snmp_poll_data';
...
SELECT
    ...
    ,CAST(ts AS STRING)                             AS ts -- Cast the TIMESTAMP(3) to STRING for the target table
    ,DATE_FORMAT(ts, 'yyyyMM')                      AS partition_month -- Directly format the TIMESTAMP column
FROM hive.snmp.snmp_poll_data4;


SET 'pipeline.name' = 'Push SNMP snmp_poll_data5 => fluss.snmp_poll_data';
...
SELECT
    ...
    ,CAST(ts AS STRING)                             AS ts -- Cast the TIMESTAMP(3) to STRING for the target table
    ,DATE_FORMAT(ts, 'yyyyMM')                      AS partition_month -- Directly format the TIMESTAMP column
FROM hive.snmp.snmp_poll_data5;


SET 'pipeline.name' = 'Push SNMP snmp_poll_data6 => fluss.snmp_poll_data';
...
SELECT
    ...
    ,CAST(ts AS STRING)                             AS ts -- Cast the TIMESTAMP(3) to STRING for the target table
    ,DATE_FORMAT(ts, 'yyyyMM')                      AS partition_month -- Directly format the TIMESTAMP column
FROM hive.snmp.snmp_poll_data6;